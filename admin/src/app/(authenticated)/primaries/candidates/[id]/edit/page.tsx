...
import type { PrimaryCandidate, ContentBlock } from '@/lib/types';
...
export default function EditCandidatePage() {
  ...
  const { data: candidate, isLoading } = useQuery({
    ...
    queryFn: () => api.get<PrimaryCandidate>(`/candidates/${candidateId}`),
  });
  ...
  useEffect(() => {
    if (candidate) {
      setForm({
        ...
        shortBio: candidate.bio || '',
        bioBlocks: candidate.bioBlocks || [],
        photoUrl: candidate.photoUrl || '',
        coverImageUrl: candidate.coverImageUrl || '',
        phone: candidate.phone || '',
        email: candidate.email || '',
        website: candidate.website || '',
        socialTwitter: candidate.socialLinks?.twitter || '',
        socialFacebook: candidate.socialLinks?.facebook || '',
        socialInstagram: candidate.socialLinks?.instagram || '',
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">עריכת מועמד</h1>
        <div className="flex gap-2">
          <Button type="button" variant="outline" onClick={() => router.back()}>
            ביטול
          </Button>
          <Button type="submit" disabled={updateMutation.isPending}>
            {updateMutation.isPending ? 'שומר...' : 'עדכן מועמד'}
          </Button>
        </div>
      </div>
