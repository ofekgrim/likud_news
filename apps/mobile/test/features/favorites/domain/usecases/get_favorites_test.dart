...
void main() {
  ...
  group('GetFavorites', () {
    test(
        'should delegate to repository.getFavorites(deviceId: ..., page: 1)',
        () async {
      // arrange
      when(() => mockRepository.getFavorites(deviceId: tDeviceId, page: 1))
          .thenAnswer((_) async => const Right(tArticles));

      // act
      await useCase(const FavoritesParams(deviceId: tDeviceId, page: 1));

      // assert
      verify(() => mockRepository.getFavorites(deviceId: tDeviceId, page: 1))
          .called(1);
    ...
    test('should pass the correct parameters', () async {
      // arrange
      when(() => mockRepository.getFavorites(deviceId: tDeviceId, page: 5))
          .thenAnswer((_) async => const Right(tArticles));

      // act
      await useCase(const FavoritesParams(deviceId: tDeviceId, page: 5));

      // assert
      verify(() => mockRepository.getFavorites(deviceId: tDeviceId, page: 5))
          .called(1);
    });
  });

  group('FavoritesParams', () {
    test('should support value equality', () {
      // arrange
      const params1 = FavoritesParams(deviceId: tDeviceId, page: 1);
      const params2 = FavoritesParams(deviceId: tDeviceId, page: 1);
    ...
    test('should not be equal when deviceId or page differs', () {
      // arrange
      const params1 = FavoritesParams(deviceId: tDeviceId, page: 1);
      const params2 = FavoritesParams(deviceId: 'other-device', page: 1);
      const params3 = FavoritesParams(deviceId: tDeviceId, page: 2);
    ...
    test('props should contain deviceId and page', () {
      // arrange
      const params = FavoritesParams(deviceId: tDeviceId, page: 3);
